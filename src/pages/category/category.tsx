...
import { addCategory, editCategory, getCategory, deleteCategory } from '@/service/category'; 
...
const Goods: React.FC = () => {
  ...
  const getData = async () => {
    setSpinning(true);
    const [, res] = await catchError<any>(getCategory());
    setSpinning(false);
    setTableData(res);
  }
  ...
  const remove = (data: IFormData) => {
    data.id && deleteCategory(data);
    getData();
  }
  ...
  const columns: ColumnsType<IFormData> = [
    ...
    },
    {
      ...
      key: 'img',
      render(data) {
        return <img src={baseUrl + data}></img>
      },
    },
    {
      title: 'operation',
  ...
  return (
    <>
      <Spin tip="数据加载中..." spinning={spinning}>
        <div className={styles.content}>
          <div style={{padding: "0 0 20px 0"}}>
            <Button type='primary' htmlType="submit" onClick={() => setOpen(true)}>添加商品类目</Button>
          </div>
      ...
      <Drawer
        ...
        >
          <Form.Item rules={[{ required: true}]} label="类目名称" name='lable'>
            <Input />
          </Form.Item>
          <Form.Item rules={[{ required: true }]} label="类目key" name='value'>
            <Input />
          </Form.Item>
          <Form.Item label="类目图片" valuePropName="fileList" getValueFromEvent={normFile} name="img">
            <Upload
